...
def get_pos_transition_matrix(pos_tags):
    ...
    pos_transition_matrix = np.divide(
        pos_transition_matrix, row_sums, where=row_sums != 0
    )
...
class TextData:
    ...
    @staticmethod
    def classify_with_vote(k_mean_vote, hierarchical_vote, autoencoder_vote, pos_vote):
        """
        Classify the text based on weighted votes where classifiers have different accuracy levels.

        Weights based on accuracy: POS > K-means > Encoder > Agglomerate

        :param k_mean_vote: Integer (0 or 1) representing the KMeans classifier vote
        :param hierarchical_vote: Integer (0 or 1) representing the Hierarchical/Agglomerate classifier vote
        :param autoencoder_vote: Integer (0 or 1) representing the Autoencoder classifier vote
        :param pos_vote: Integer (0 or 1) representing the POS Transition Matrix classifier vote
        :return: An integer (0 or 1) representing the classification (AI or HUMAN).
        """
        # Assign weights based on known accuracy
        pos_weight = 4  # Simple_POS_Transition_Matrix (most accurate)
        if pos_vote == 0:
            # AI vote so Autoencoder get more power
            autoencoder_weight = 3
            kmean_weight = 2
            hierarchical_weight = 1
        else:
            # Human vote so Hierarchical get more power
            kmean_weight = 2
            autoencoder_weight = 1
            hierarchical_weight = 3

        # Calculate weighted sum
        weighted_sum = (
            k_mean_vote * kmean_weight
            + hierarchical_vote * hierarchical_weight
            + autoencoder_vote * autoencoder_weight
            + pos_vote * pos_weight
        )

        # Calculate total weight
        total_weight = (
            kmean_weight + hierarchical_weight + autoencoder_weight + pos_weight
        )
    ...
    def __str__(self):
        return (
            f"TextData(id={self.id}, content={self.content}, pos_tags={self.pos_tags}, "
...
def create_text_data_list(documents, text_type):
    ...
    for doc in documents:
        text_data = TextData(
            doc["_id"],
            doc["text-embedding-005-classification"],
            doc["text-embedding-005-clustering"],
            doc["Content"],
            doc["wordFamilyContent"],
            text_type,
            doc["Model"] if text_type == AI else None,
        )
