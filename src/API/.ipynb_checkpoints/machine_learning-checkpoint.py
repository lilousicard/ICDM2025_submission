...
def apply_pca(pca_model, text_data_list, feature, pca_feature):
    ...
    if feature_array.shape[1] != pca_model.n_features_in_:
        raise ValueError(f"Feature array shape {feature_array.shape[1]} does not match "
...
def k_mean_cluster(text_data_list, feature, data_type_field, n_clusters, n_init=10, algorithm='auto', random_state=42, file_name=""):
    """
    ...
    # Extract the feature from the text data list
    feature_list = [getattr(text_data, feature) for text_data in text_data_list]
    data_type_list = [getattr(text_data, data_type_field) for text_data in text_data_list]
    ...
    kmeans = KMeans(n_clusters=n_clusters, n_init=n_init, random_state=random_state, algorithm=algorithm).fit(feature_array)
    ...
    cluster_majorities = {label: counts.most_common(1)[0][0] for label, counts in cluster_counts.items()}
    ...
    if len(set(cluster_majorities.values())) == 1:
        ...
        print("Changing the label of the cluster with the most data points to the opposite label.")
...
def k_mean_cluster_test(kmeans_model, text_data_list, feature, data_type_field, cluster_majorities):
    """
...
def k_mean_cluster_fit_predict(kmeans_model, text_data_obj, feature, cluster_majorities):
    """
    ...
    feature_array = np.array(getattr(text_data_obj, feature)).reshape(1, -1)

    # Predict the cluster using the trained KMeans model
    cluster_label = kmeans_model.predict(feature_array)[0]

    # Update `vote` for the text_data object based on precomputed cluster_majorities
    predicted_label = cluster_majorities[cluster_label]
    method_name = "KMeans" + str(kmeans_model.n_clusters) + "_" + feature
    text_data_obj.add_vote(method_name, predicted_label)
    return predicted_label


def save_kmeans_model(kmeans, feature_array, cluster_majorities, filename="kmeans_model.pkl"):
    """
...
def hierarchical_cluster(text_data_list, feature, data_type_field, n_clusters, distance_threshold = None, file_name=""):
    """
    ...
    generate_classification_report(data_type_list, predicted_labels, target_names=["AI", "HUMAN"])
    ...
    # Save the model if a filename is provided
    if file_name:
        save_hierarchical_model(agg_cluster, feature_array, cluster_majorities, cluster_centroids, filename=file_name)

    return agg_cluster, feature_array, cluster_majorities, cluster_centroids


def hierarchical_cluster_test(agg_cluster_model, text_data_list, feature, data_type_field, cluster_majorities,
                              cluster_centroids):
    """
    ...
    for i, vec in enumerate(feature_array):
        ...
        distances = {label: norm(vec - centroid) for label, centroid in cluster_centroids.items()}
...
def hierarchical_cluster_fit_predict(agg_cluster_model, text_data_obj, feature, cluster_majorities, cluster_centroids):
    """
    ...
    # Compute Euclidean distances to each centroid
    distances = {label: norm(feature_array - centroid) for label, centroid in cluster_centroids.items()}
    assigned_cluster = min(distances, key=distances.get)
...
def save_hierarchical_model(agg_cluster, feature_array, cluster_majorities, cluster_centroids, filename="hierarchical_model.pkl"):
    """
...
def create_dataloader(text_data_list, feature_field, batch_size):
    ...
    """
    tensor = torch.tensor([getattr(data, feature_field) for data in text_data_list], dtype=torch.float32)
...
def autoencoder_classify_text(embedding, h_encoder, AI_encoder, h_criterion, AI_criterion):
    """
...
def save_autoencoders(h_encoder, AI_encoder, h_optimizer, AI_optimizer, input_dim, latent_dim,
...
def load_autoencoders(filename="autoencoders.pth"):
    ...
    h_encoder, _, h_optimizer = create_autoencoder(checkpoint["input_dim"], checkpoint["latent_dim"])
    AI_encoder, _, AI_optimizer = create_autoencoder(checkpoint["input_dim"], checkpoint["latent_dim"])

    # Load saved states
    h_encoder.load_state_dict(checkpoint["h_encoder_state"])
    AI_encoder.load_state_dict(checkpoint["AI_encoder_state"])
    h_optimizer.load_state_dict(checkpoint["h_optimizer_state"])
    AI_optimizer.load_state_dict(checkpoint["AI_optimizer_state"])

    print("Autoencoders successfully loaded")
    return h_encoder, AI_encoder, h_optimizer, AI_optimizer, checkpoint
...
def plot_actual_label(text_data_list, feature, title="Actual Classification"):
    ...
    labels = [text_data.text_type for text_data in text_data_list]  # Assuming text_type stores AI or HUMAN constants
    ...
    if feature_array.shape[1] < 2:
        raise ValueError("Feature array must have at least two dimensions for plotting.")
    ...
    plt.scatter(feature_array[:, 0], feature_array[:, 1], c=point_colors, alpha=0.5, label='Data Points')

    # Create a custom legend
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=10, label='AI'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='orange', markersize=10, label='HUMAN')
    ]
...
def plot_actual_label_3d(text_data_list, feature, title="Actual Classification (3D)"):
    ...
    if feature_array.shape[1] < 3:
        raise ValueError("Feature array must have at least three dimensions for 3D plotting.")
...
def plot_latest_result(data_list, method_name, feature_field, title="Latest Classification Result"):
    """
    ...
    latest_votes = [data.votes[method_name] if method_name in data.votes else None for data in data_list]

    # Filter out data points without votes
    filtered_data = [(feat, vote) for feat, vote in zip(features, latest_votes) if vote is not None]
    ...
    # Create scatter plot
    plt.figure(figsize=(10, 6))
    scatter = plt.scatter(feature_array[:, 0], feature_array[:, 1], c=point_colors, alpha=0.5)
